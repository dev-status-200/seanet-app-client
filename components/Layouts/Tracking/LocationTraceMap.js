...
const LocationTraceMap = ({selectedUser}) => {
	...
	useEffect(() => {
		...
		setTimeout(async function(){
			await axios.get(process.env.NEXT_PUBLIC_SEANET_SYS_GET_RIDER_ROUTE_GET,{ headers:{ id:`${selectedUser.id}` }}).then((x)=>{ getMatch(x.data.result.Item.routes,true) })
		  }, 1000)

		const interval = setInterval(async() => {
			await axios.get(process.env.NEXT_PUBLIC_SEANET_SYS_GET_RIDER_ROUTE_GET,{ headers:{ id:`${selectedUser.id}` }}).then((x)=>{ getMatch(x.data.result.Item.routes,false) })
		}, 5000);
...
export default LocationTraceMap