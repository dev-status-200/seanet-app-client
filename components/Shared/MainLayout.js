...
const MainLayout = ({children}) => {
  ...
  const values = () => {
    let value = '1';
      ...
      else if(router.pathname === '/paymentRequest'){ value='7' }
      else { value='null' }
    return value
  }
    ...
    <Layout className='layoutStyles'>
      <Sider trigger={null} collapsible collapsed={collapsed}>
        ...
        <span className={'darkTheme headr-icon'}>
          ...
          {permissions.f5=='1'&& <Menu.Item key="7" icon={<CreditCardOutlined />}>
            <Link href="/paymentRequest"><a style={{textDecoration:'none'}}>Requests</a></Link>
          </Menu.Item>
          }
        </Menu>
      </Sider>
